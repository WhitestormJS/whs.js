...
Physijs.Scene = function (params) {
  ...
  this._worker = new _inlineWorker2.default(require('./worker.js'));
...
Physijs.Scene.prototype._updateScene = function (data) {
  ...
  if (SUPPORT_TRANSFERABLE) this._worker.transferableMessage(data.buffer, [data.buffer]); // Give the typed array back to the worker
      ...
      for (var _j = 0; _j < collisions[id1].length; _j++) {
        ...
        if (_object2) {
          ...
          if (_object._physijs.touches.indexOf(id2) === -1) {
            ...
            temp1Vector3.subVectors(_object.getLinearVelocity(), _object2.getLinearVelocity());
            temp1 = temp1Vector3.clone();

            temp1Vector3.subVectors(_object.getAngularVelocity(), _object2.getAngularVelocity());
            ...
            _object.dispatchEvent('collision', _object2, temp1, temp2, temp1Vector3);
          }
        }
      }
...
Physijs.HeightfieldMesh = function (geometry, material, mass, xdiv, ydiv) {
  ...
  for (var i = 0; i < geometry.vertices.length; i++) {
    var a = i % this._physijs.xpts;
    var b = Math.round(i / this._physijs.xpts - i % this._physijs.xpts / this._physijs.xpts);
...
// Physijs.Vehicle
Physijs.Vehicle = function (mesh) {
  var tuning = arguments.length <= 1 || arguments[1] === undefined ? new Physijs.VehicleTuning() : arguments[1];
...
Physijs.VehicleTuning = function () {
