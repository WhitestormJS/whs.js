...
function generateSourceFiles(sourceFiles, encoding) {
  ...
  Object.keys(sourceFiles).forEach(function(file) {
    ...
    var path = res.substring(res.indexOf('src'), res.length);

    var sourceOutfile = repo + path // helper.getUniqueFilename(sourceFiles[file].shortened)
...
exports.publish = function(taffyData, opts, tutorials) {
  ...
  var markdowns = env.opts._ ? env.opts._.slice(0).concat(env.conf.source.include) : [];
  ...
  var fix = function (sh) {
    ...
    + sh.slice(0, sh.indexOf('/') + 1)
    + '</span>'
    + sh.slice(sh.indexOf('/') + 1, sh.length)
  }
